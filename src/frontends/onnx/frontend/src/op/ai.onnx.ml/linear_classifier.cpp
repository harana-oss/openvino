...
ov::Output<ov::Node> ensure_float(const ov::Output<ov::Node>& inp) {
    if (inp.get_element_type() == ov::element::f32)
        return inp;
...
ov::OutputVector linear_classifier(const ov::frontend::onnx::Node& node) {
    ...
    auto coeff_const = ov::op::v0::Constant::create(ov::element::f32, ov::Shape{k, n_features}, coefficients);
    auto order = ov::op::v0::Constant::create(ov::element::i64, ov::Shape{2}, {1,0});
    auto coeff_T = std::make_shared<ov::op::v1::Transpose>(coeff_const, order); // [n_features, k]
...
// Registration moved to default domain file: src/op/linear_classifier.cpp
} // namespace opset_1
